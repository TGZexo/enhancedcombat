...
package net.mcreator.enhancedcombat.keybind;
...
import net.mcreator.enhancedcombat.procedures.DashProcedureProcedure;
...
public class DashButtonKeyBinding extends EnhancedcombatModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public DashButtonKeyBinding(EnhancedcombatModElements instance) {
		super(instance, 1);
	...
	public void initElements() {
		keys = new KeyBinding("key.enhancedcombat.dash_button", GLFW.GLFW_KEY_F, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					EnhancedcombatMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
